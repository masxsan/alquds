...
function initializeApp() {
    ...
    // Initialize jumat staff data
    initJumatStaffData();
    
    // Initialize jumat staff display
    updateJumatStaffDisplay();
    
    // NOTE: Jumat schedule functionality has been removed from the application
    // The following code remains for reference purposes only
    /*
...
function setupAdminPanel() {
    ...
    // Save jumat staff
    document.getElementById('save-jumat-staff')?.addEventListener('click', saveJumatStaff);
    
    // Add jumat staff
    document.getElementById('add-jumat-staff')?.addEventListener('click', addNewJumatStaff);
...
// Initialize jumat staff data
function initJumatStaffData() {
    // Check if jumat staff data exists in localStorage, if not, create default
    let savedJumatStaff = localStorage.getItem('jumatStaff');
    if (!savedJumatStaff) {
        const defaultJumatStaff = [
            { id: 1, day: 'Jumat, 03 Jan 2025', imam: 'Ust. Ahmad Rifai', khotib: 'Ust. Muhammad Ali', muadzin: 'Ust. Budi Santoso' },
            { id: 2, day: 'Jumat, 10 Jan 2025', imam: 'Ust. Hasan Basri', khotib: 'Ust. Yusuf Mansur', muadzin: 'Ust. Joko Widodo' },
            { id: 3, day: 'Jumat, 17 Jan 2025', imam: 'Ust. Abdul Karim', khotib: 'Ust. Salman Al-Farisi', muadzin: 'Ust. Ridwan Kamil' },
            { id: 4, day: 'Jumat, 24 Jan 2025', imam: 'Ust. Syahrul Yasin', khotib: 'Ust. Faisal Arafat', muadzin: 'Ust. Agus Harimurti' },
            { id: 5, day: 'Jumat, 31 Jan 2025', imam: 'Ust. Rizki Ramadhan', khotib: 'Ust. Dani Pedrosa', muadzin: 'Ust. Tommy Soeharto' },
            { id: 6, day: 'Jumat, 07 Feb 2025', imam: 'Ust. Wildan Fahmi', khotib: 'Ust. Arif Rahman', muadzin: 'Ust. Susilo Bambang' }
        ];
        localStorage.setItem('jumatStaff', JSON.stringify(defaultJumatStaff));
    }
    
    // Load jumat staff data into the form
    loadJumatStaffInfo();
}

// Load jumat staff info into admin panel
function loadJumatStaffInfo() {
    const container = document.getElementById('jumat-staff-container');
    if (!container) return;
    
    const jumatStaffData = JSON.parse(localStorage.getItem('jumatStaff') || '[]');
    
    container.innerHTML = ''; // Clear existing content
    
    if (jumatStaffData.length === 0) {
        container.innerHTML = '<p>Tidak ada data petugas Jumat. Silakan tambah data menggunakan tombol di bawah.</p>';
        return;
    }
    
    // Create table for jumat staff data
    const table = document.createElement('table');
    table.className = 'jumat-staff-edit-table';
    table.style.width = '100%';
    table.style.borderCollapse = 'collapse';
    ...
    // Add jumat staff rows
    jumatStaffData.forEach((item, index) => {
        const row = document.createElement('tr');
        row.dataset.id = item.id;
        row.innerHTML = `
            <td style="border: 1px solid #ddd; padding: 8px;">${index + 1}</td>
            <td style="border: 1px solid #ddd; padding: 8px;">
                <input type="text" class="jumat-staff-day" value="${item.day}" style="width: 100%; padding: 4px;">
            </td>
            <td style="border: 1px solid #ddd; padding: 8px;">
                <input type="text" class="jumat-staff-imam" value="${item.imam}" style="width: 100%; padding: 4px;">
            </td>
            <td style="border: 1px solid #ddd; padding: 8px;">
                <input type="text" class="jumat-staff-khotib" value="${item.khotib}" style="width: 100%; padding: 4px;">
            </td>
            <td style="border: 1px solid #ddd; padding: 8px;">
                <input type="text" class="jumat-staff-muadzin" value="${item.muadzin}" style="width: 100%; padding: 4px;">
            </td>
            <td style="border: 1px solid #ddd; padding: 8px;">
                <button class="delete-jumat-staff-btn" onclick="deleteJumatStaff(${item.id})" style="background-color: #f44336; color: white; border: none; padding: 4px 8px; cursor: pointer; border-radius: 4px;">Hapus</button>
            </td>
        `;
        table.appendChild(row);
    });
...
// Save jumat staff data
function saveJumatStaff() {
    const rows = document.querySelectorAll('#jumat-staff-container table tr:not(:first-child)');
    
    const updatedJumatStaffData = [];
    
    rows.forEach(row => {
        const inputs = row.querySelectorAll('input');
        if (inputs.length >= 4) { // day, imam, khotib, muadzin
            const id = parseInt(row.dataset.id);
            ...
            if (day && imam && khotib && muadzin) { // Only save if all fields are filled
                updatedJumatStaffData.push({ id, day, imam, khotib, muadzin });
            }
        }
    });
    
    // Save to localStorage
    localStorage.setItem('jumatStaff', JSON.stringify(updatedJumatStaffData));
    
    // Update the display on the main page
    updateJumatStaffDisplay();
    
    alert('Data Petugas Sholat Jumat berhasil disimpan!');
}

// Add new jumat staff entry
function addNewJumatStaff() {
    const container = document.getElementById('jumat-staff-container');
    if (!container) return;
    
    const jumatStaffData = JSON.parse(localStorage.getItem('jumatStaff') || '[]');
    const newId = jumatStaffData.length > 0 ? Math.max(...jumatStaffData.map(f => f.id)) + 1 : 1;
    ...
    jumatStaffData.push(newData);
    localStorage.setItem('jumatStaff', JSON.stringify(jumatStaffData));
    
    // Reload the form to show the new entry
    loadJumatStaffInfo();
}

// Delete jumat staff entry
function deleteJumatStaff(id) {
    if (confirm('Apakah Anda yakin ingin menghapus data petugas Jumat ini?')) {
        let jumatStaffData = JSON.parse(localStorage.getItem('jumatStaff') || '[]');
        jumatStaffData = jumatStaffData.filter(item => item.id !== id);
        localStorage.setItem('jumatStaff', JSON.stringify(jumatStaffData));
...
// Update jumat staff display on main page
function updateJumatStaffDisplay() {
    const jumatStaffData = JSON.parse(localStorage.getItem('jumatStaff') || '[]');
    const jumatStaffTableBody = document.querySelector('.jumat-staff-table tbody');
    
    if (jumatStaffTableBody) {
        jumatStaffTableBody.innerHTML = ''; // Clear existing rows
        
        // Show all jumat staff data in the table
        if (jumatStaffData.length > 0) {
            jumatStaffData.forEach((staff, index) => {
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${index + 1}</td>
                    <td>${staff.day}</td>
                    ...
                    <td>${staff.muadzin}</td>
                `;
                jumatStaffTableBody.appendChild(row);
            });
        } else {
            // If no staff data, show default rows
            for (let i = 1; i <= 6; i++) {
                const row = document.createElement('tr');
                row.innerHTML = `
                    <td>${i}</td>
    ...
    let savedJumat = localStorage.getItem('jumatSchedule');
    if (!savedJumat) {
        const defaultJumat = [
            ...
            { id: 4, date: 'Jumat, 16 Mei 2025', imam: 'Ust. Joko Widodo', khotib: 'Ust. Prabowo Subianto', muadzin: 'Ust. Megawati Sukarnoputri' },
            { id: 5, date: 'Jumat, 23 Mei 2025', imam: 'Ust. Anies Baswedan', khotib: 'Ust. Ganjar Pranowo', muadzin: 'Ust. Erick Thohir' }
        ];
        localStorage.setItem('jumatSchedule', JSON.stringify(defaultJumat));
    } else {
        // Check if we need to upgrade from old 3-row format to 5-row format
        const jumatData = JSON.parse(savedJumat);
        if (jumatData.length < 5) {
            // Add 2 more default entries to reach 5 total
            if (jumatData.length === 3) {
                jumatData.push(
                    { id: 4, date: 'Jumat, 16 Mei 2025', imam: 'Ust. Joko Widodo', khotib: 'Ust. Prabowo Subianto', muadzin: 'Ust. Megawati Sukarnoputri' },
                    { id: 5, date: 'Jumat, 23 Mei 2025', imam: 'Ust. Anies Baswedan', khotib: 'Ust. Ganjar Pranowo', muadzin: 'Ust. Erick Thohir' }
                );
...
function updateJumatScheduleDisplay() {
    ...
    if (jumatTableBody) {
        ...
        // Display all existing data, or create 5 default entries if no data exists
        if (jumatData.length === 0) {
            // If no data exists, create 5 default entries
            jumatData = [];
            for (let i = 1; i <= 5; i++) {
                jumatData.push({
                    id: i,
                    date: `Jumat, ${i*7} Mei 2025`,
                    imam: `Ust. Imam ${i}`,
                    khotib: `Ust. Khotib ${i}`,
                    muadzin: `Ust. Muadzin ${i}`
                });
            }
        }
        // If data exists, display all of it (no need to pad to 5 rows)
...
function initStaffData() {
    ...
    if (!savedStaff) {
        const defaultStaff = [
            ...
            { id: 8, day: 'Ahad', imam: 'Ust. Abdullah', muadzin: 'Ust. Hamzah' }
        ];
        localStorage.setItem('staffInfo', JSON.stringify(defaultStaff));
    } else {
        // If staff data exists, check if it has the Ahad entry, if not, add it
        let staffData = JSON.parse(savedStaff);
        const hasAhadEntry = staffData.some(staff => staff.day === 'Ahad');
        if (!hasAhadEntry) {
            const newAhadEntry = { id: 8, day: 'Ahad', imam: 'Ust. Abdullah', muadzin: 'Ust. Hamzah' };
            staffData.push(newAhadEntry);
...
function playAdhan(prayerName) {
    ...
    // Play the adzan audio file
    console.log(`Adhan for ${prayerName} should be played now`);
    
    // Play the adzan audio from the specified location
    try {
        const audio = new Audio('asset/audio/adzan.mp3');
        ...
        if (playPromise !== undefined) {
            playPromise.catch(error => {
                console.error("Error playing adzan audio:", error);
                console.log("Audio autoplay was prevented by browser policies");
                
                // Try to play again after a brief moment to handle autoplay restrictions
                setTimeout(() => {
                    audio.play().catch(retryError => {
                        console.error("Retry error playing adzan audio:", retryError);
                    });
                }, 100);
            });
        }
